...
//Constructor
Buffer::Buffer(size_t size): mWriteIndex(0), mReadIndex(0), mWriteIndexHeader(0), mReadIndexHeader(0)
{
	mBuffer.resize(size);
}
...
void Buffer::WriteInt32ToHeader(int value)
{
	headerBuffer[mWriteIndexHeader] = value;
	headerBuffer[mWriteIndexHeader + 1] = value >> 8;
	headerBuffer[mWriteIndexHeader + 2] = value >> 16;
	headerBuffer[mWriteIndexHeader + 3] = value >> 24;
...
int Buffer::ReadInt32LEHeader(void)
{
	uint32_t value = headerBuffer[mReadIndexHeader];
	value |= headerBuffer[mReadIndexHeader + 1] << 8;
	value |= headerBuffer[mReadIndexHeader + 2] << 16;
	value |= headerBuffer[mReadIndexHeader + 3] << 24;
